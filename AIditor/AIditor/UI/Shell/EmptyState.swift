...
//  Zdroj zadání: `design_handoff_aiditor_ui/README.md`, obrazovka 2 (varianta 3a).
//
...
struct DropZoneContent: View {
    ...
    private var box: some View {
        VStack(spacing: 14) {
            ...
            Text("AIditor si u každého souboru změří skutečné časování (VFR poznáš "
        ...
        .background(
            RoundedRectangle(cornerRadius: 14)
                .fill(isTargeted ? AIditorUI.accent.opacity(0.06) : Color.white.opacity(0.014))
        )
        .overlay(
            RoundedRectangle(cornerRadius: 14)
                .strokeBorder(isTargeted ? AIditorUI.accent : Color(aiditorHex: 0x33373E),
    ...
    private var tiles: some View {
        HStack(alignment: .bottom, spacing: 6) {
            tile(width: 44, height: 30,
                 colors: [Color(aiditorHex: 0x3A4250), Color(aiditorHex: 0x1A1D22)],
                 border: Color(aiditorHex: 0x33373E))
            tile(width: 56, height: 38,
                 colors: [Color(aiditorHex: 0x465060), Color(aiditorHex: 0x1C2129)],
                 border: AIditorUI.borderActive)
            ZStack {
                RoundedRectangle(cornerRadius: 4).fill(Color(aiditorHex: 0x1E2C26))
                waveHint
            }
            .frame(width: 44, height: 30)
            .overlay(RoundedRectangle(cornerRadius: 4)
                .strokeBorder(Color(aiditorHex: 0x33373E), lineWidth: 1))
        }
    }
    ...
    private var waveHint: some View {
        HStack(spacing: 2) {
            ForEach(0..<7, id: \.self) { _ in
                Color(aiditorHex: 0x78DCAA, opacity: 0.55).frame(width: 1)
            }
        }
        .frame(height: 10)
    }
    ...
    private func zoneButton(_ title: String, isPrimary: Bool,
        ...
        Button(action: action) {
            Text(title)
                ...
                .foregroundStyle(isPrimary ? AIditorUI.textPrimary : Color(aiditorHex: 0xC9CCD1))
                .padding(.horizontal, 14)
                .frame(height: 30)
                .background(RoundedRectangle(cornerRadius: 8)
                    .fill(isPrimary ? AIditorUI.surfaceControlActive : AIditorUI.surfaceControl))
                .overlay(RoundedRectangle(cornerRadius: 8)
                    .strokeBorder(isPrimary ? AIditorUI.borderActive : AIditorUI.borderStrong,
                                  lineWidth: 1))
        }
        .buttonStyle(.plain)
    }
        ...
        VStack(alignment: .leading, spacing: 6) {
            ...
            HStack(spacing: 8) {
                dot(model.transcription.modelSizeBytes == nil
                    ? AIditorUI.textDisabled : AIditorUI.ok)
                Text(whisperText)
                    .font(AIditorUI.Font.status)
                    .foregroundStyle(AIditorUI.textTertiary)
                Spacer(minLength: 4)
    ...
    var body: some View {
        Button(action: action) {
                ...
                VStack(alignment: .leading, spacing: 2) {
                    Text(recent.name)
                        ...
                        .foregroundStyle(recent.isOffline
                                         ? AIditorUI.textSecondary : AIditorUI.textPrimary)
                        .lineLimit(1)
            ...
            .background(RoundedRectangle(cornerRadius: 9)
                .fill(recent.isOffline ? Color.clear : AIditorUI.surfaceControlActive))
            .overlay(RoundedRectangle(cornerRadius: 9)
                .strokeBorder(recent.isOffline ? Color.clear : Color(aiditorHex: 0x33373E),
                              lineWidth: 1))
        }
        .buttonStyle(.plain)
        ...
        ZStack {
            RoundedRectangle(cornerRadius: 4)
                .fill(recent.isOffline
                      ? LinearGradient(colors: [Color(aiditorHex: 0x1C1F24),
                                                Color(aiditorHex: 0x1C1F24)],
                                       startPoint: .top, endPoint: .bottom)
                      : LinearGradient(colors: [Color(aiditorHex: 0x463F38),
                                                Color(aiditorHex: 0x1A1D22)],
                                       startPoint: .topLeading, endPoint: .bottomTrailing))
            if recent.isOffline {
                Text("offline")
                    .font(.system(size: 8))
                    .foregroundStyle(AIditorUI.textDisabled)
            }
        }
...
struct RecoveryBar: View {
    ...
    var body: some View {
        HStack(spacing: 12) {
            Circle().fill(AIditorUI.warn).frame(width: 7, height: 7)
            (Text("Aplikace minule neskončila uložením. Záloha neuloženého projektu je z ")
                .foregroundStyle(Color(aiditorHex: 0xFFD7A0))
             + Text(Self.stamp.string(from: offer.modifiedAt))
                .foregroundStyle(.white)
             + Text(" · " + clipText).foregroundStyle(Color(aiditorHex: 0xFFD7A0)))
                .font(.system(size: 12))
                .lineLimit(1)
            ...
            Button {
                ...
                Text("Obnovit zálohu")
                    ...
                    .background(RoundedRectangle(cornerRadius: 6)
                        .fill(AIditorUI.warn.opacity(0.16)))
                    .overlay(RoundedRectangle(cornerRadius: 6)
                        .strokeBorder(AIditorUI.warn.opacity(0.55), lineWidth: 1))
            }
            .buttonStyle(.plain)
        ...
        .padding(.vertical, 9)
        .frame(maxWidth: .infinity)
        .background(AIditorUI.warn.opacity(0.10))
        .overlay(alignment: .bottom) {
            AIditorUI.warn.opacity(0.30).frame(height: 1)
        }
    }
