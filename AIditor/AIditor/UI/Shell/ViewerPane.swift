    ...
    var body: some View {
        ZStack {
            ...
            .padding(model.chromeHidden || model.previewFullscreen
                     ? 0 : AIditorUI.Metric.viewerPadding)
    ...
    var body: some View {
        HStack(spacing: 6) {
            if timeline.project.usesProxies {
                chip("Proxy 1/2", color: AIditorUI.textPrimary)
            }
            if let ramp = rampText {
                chip(ramp, color: AIditorUI.rampCurve)
            }
...
private struct LoudnessGauge: View {
    ...
    var body: some View {
        VStack(alignment: .trailing, spacing: 4) {
            ...
            Text(readout.map { Self.decimal($0.truePeakDB) + " TP" } ?? "— TP")
                .font(AIditorUI.Font.chip)
                .foregroundStyle(readout == nil ? AIditorUI.textTertiary : AIditorUI.warn)
        }
        .padding(.horizontal, 7)
        .padding(.vertical, 6)
        .background(
    ...
    private var bar: some View {
        RoundedRectangle(cornerRadius: 3)
            .fill(readout == nil ? AIditorUI.textTertiary.opacity(0.35) : AIditorUI.ok)
            .frame(width: 6)
    }
    ...
    var body: some View {
        HStack(spacing: 10) {
            ...
            Button {
                ...
                ZStack {
                    Circle().fill(AIditorUI.textPrimary)
                    Image(systemName: controller.isPlaying ? "pause.fill" : "play.fill")
                        .font(.system(size: 15, weight: .semibold))
                        .foregroundStyle(AIditorUI.surfaceRail)
                }
                .frame(width: AIditorUI.Metric.transportPlayButton,
                       height: AIditorUI.Metric.transportPlayButton)
            }
            .buttonStyle(.plain)
                    ...
                    .background(
                        RoundedRectangle(cornerRadius: AIditorUI.Metric.chipRadius)
                            .fill(controller.isSteppingFallback
                                  ? AIditorUI.warn.opacity(0.14)
                                  : Color.white.opacity(0.08))
                    )
            }
        }
        .padding(.horizontal, 14)
        .frame(height: AIditorUI.Metric.transportPillHeight)
        .background(
            RoundedRectangle(cornerRadius: AIditorUI.Metric.transportPillRadius)
                .fill(Color(aiditorHex: 0x121418, opacity: 0.92))
        )
        .overlay(
            RoundedRectangle(cornerRadius: AIditorUI.Metric.transportPillRadius)
