...
- **⚠️ Vnořený `ObservableObject` se musí odebírat ZVLÁŠŤ.** `TimelineController` a `TranscriptionService` žijí uvnitř `AppModelu`; view, které pozoruje jen model, jejich změny NEVIDÍ. Projekt na to narazil dvakrát (F11 u `selectedTitle`, F18/M11 u `selectedSpeech` — a tam to bylo nebezpečné: pole s textem úseku zůstalo prázdné a prázdný text úsek MAŽE). Každý panel si má vzít `@ObservedObject` na to, co doopravdy čte.
