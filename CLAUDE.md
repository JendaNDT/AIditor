...
- **⚠️ Průběžné úseky z WhisperKitu: `segmentDiscoveryCallback` na INSTANCI, a časy z něj neber.** Ověřeno ve zdrojácích balíčku (F18/M11): ① parametr `segmentCallback:` u `transcribe(audioArray:…)` se při VAD chunkingu **nepoužije** — batchovaná cesta si staví closure nad instanční vlastností `segmentDiscoveryCallback`; ② v té closure se posouvá jen pole `seek` (offset kusu ve vzorcích), **ne `start`/`end`**, takže časy jsou relativní ke kusu. Průběžný seznam je proto NÁHLED (text + pozice ze `seek`, který absolutní je) a autoritativní úseky se berou z návratové hodnoty `transcribe`.
- **⚠️ Vnořený `ObservableObject` se musí odebírat ZVLÁŠŤ.** `TimelineController` a `TranscriptionService` žijí uvnitř `AppModelu`; view, které pozoruje jen model, jejich změny NEVIDÍ. Projekt na to narazil třikrát (F11 u `selectedTitle`, F18/M11 u `selectedSpeech` — a tam to bylo nebezpečné: pole s textem úseku zůstalo prázdné a prázdný text úsek MAŽE; F18 znovu u lišty osy). Každý panel si má vzít `@ObservedObject` na to, co doopravdy čte.
  ⚠️ **Ale `@ObservedObject` na CELÝ `TimelineController` je druhá past.** Tepe na něm `playhead` (30×/s během přehrávání) a `interaction` (každý pohyb myší při tažení), takže view, které z controlleru čte jen nastavení, by se překreslovalo celé přehrávání i celé tažení. Vzorec z opravy lišty osy: **úzké zrcadlo** (`TimelineBarState`) s cílenými odběry a `removeDuplicates()`, jako to dělá `TimelinePane`. Vlastní malý `ObservableObject`, ne `@Published` pole na `AppModelu` — tam by změna překreslovala celou skořápku. Zápis jde dál napřímo do controlleru: **psát se smí, pozorovat ne.** Změřeno v `--layers-check` části C: 60 posunů hlavy = 0 překreslení lišty, controller se přitom ohlásil 59×.
- **⚠️ Snímky v souboru se počítají VZORKY, ne buffery.** `AVAssetReaderTrackOutput` vydává i buffery **bez dat** — naměřeno 30. 07. 2026 na dvou exportech: 94 bufferů / 90 vzorků a 34 / 30, pokaždé čtyři navíc. Kdo počítá volání `copyNextSampleBuffer()`, dostane víc snímků, než soubor má, a bude tu chybu hledat v exportu (`--export-ui-check` na to naletěl). Správně `CMSampleBufferGetNumSamples`; kdo bere obraz (`CMSampleBufferGetImageBuffer`), je imunní, protože prázdný buffer přeskočí.
- **⚠️ Snímky okna si dělá aplikace SAMA, ne `screencapture`.** `ExportUIChecks.writeWindowSnapshot` kreslí `NSView.cacheDisplay(in:to:)` do PNG v kontejneru. `screencapture` fotí, co je na obrazovce — a nad oknem zůstává Finder, který otevřely dřívější kontroly (`activateFileViewerSelecting`); `NSApp.activate(ignoringOtherApps:)` ho nepřebije. Snímek z hierarchie se na z-order neptá.
...
- **⚠️ Fullscreen náhled je TÁŽ hierarchie bez chrome, ne druhé okno.** `previewFullscreen` (F18/M13) odebírá skořápku vzorcem `chromeHidden` z M1, takže `PlayerView` zůstává na témže místě stromu — přechod tam a zpět nezasekne přehrávání a hlava zůstane na svém snímku (změřeno za běžícího přehrávání: `rate` 1,0 → 1,0). Druhé okno by znamenalo druhý `AVPlayerView` a stěhování přehrávače mezi okny; to je přesně cesta, na které projekt jednou strávil den honěním „černého náhledu".
- **⚠️ Overlay, který se schovává nečinností, potřebuje `NSTrackingArea` — a ta se musí ověřit ČÍSLY.** SwiftUI `onHover` říká jen „uvnitř / venku", ne kde je kurzor; sledovač je proto `NSView` s `hitTest` na `nil` (aby nebral kliknutí tlačítkům overlaye). `--fullscreen-ui-check` se ptá na počet oblastí a jejich volby (`mouseMoved`, `activeInKeyWindow`) — bez nich se `mouseMoved` nikdy nezavolá a overlay zůstane viset navždy, což na snímku vypadá správně.
- **⚠️ Rozměry z HTML prototypu nejsou rozměry z AppKitu.** Timecode 22 mono má v návrhu šířku 132 bodů; SF Mono potřebuje na `00:00:04:04` ~150 a zbytek si zalomí na druhý řádek (F18/M13, chytil snímek okna). Kde návrh dává šířku textu, ber ji jako minimum a nech písmo rozhodnout — čísla to nikdy neuvidí.
- **⚠️ Globální statistiky mezipaměti mísí požadavky VŠECH view, i těch, na která se zrovna nekoukáš.** `--thumb-check` tři moduly padal na „vygenerováno 7 dlaždic" v druhém průchodu, přestože kód byl v pořádku: od F18/M9 si dlaždice žádá i knihovna médií (hrana 104) a kontrola izolovala jen osu. Kdo měří store, musí odstavit KAŽDÉHO žadatele — a když kontrola spadne, první otázka je „platí to i bez mého modulu?" (ověřeno stashnutím celého M13).
- **⚠️ Cíl přetažení souborů: obsah zóny musí viset UVNITŘ registrovaného `NSView`, ne nad ním.** AppKit hledá cíl hit testem od nejhlubšího pohledu pod kurzorem NAHORU po hierarchii. SwiftUI obsah položený v `ZStacku` NAD terčem není jeho potomek — hit test skončí na hostitelském pohledu SwiftUI, který registrovaný není, a nad textem a tlačítky drop tiše přestane fungovat. Řešení z F18/M12: terč hostí obsah přes vnořený `NSHostingView` (`FileDropView`), takže hit test skončí vždy uvnitř něj. **Změřeno, ne odhadnuto** — `--empty-start-check` se ptá `window.contentView.hitTest` na střed zóny a hlásí, co dostal (`NSHostingView<DropZoneContent>`). Vedlejší přínos: kontrola pak jde skutečnou cestou protokolu, kterou u SwiftUI `.onDrop` nasimulovat nejde.
- **⚠️ Přetažené URL NENÍ security-scoped.** Drop dává přístup jen na dobu operace a `startAccessingSecurityScopedResource()` na něm vrací `false`. Trvalý přístup (import, který přežije restart) dá jedině bookmark vyrobený HNED, dokud výjimka platí, a rovnou rozbalený zpátky — `MediaImporter.adopt(dropped:)`. Kdo se spolehne na `compactMap { beginAccess($0) }` jako u `NSOpenPanel`, zahodí všechny přetažené soubory.
- **⚠️ SwiftUI `ScrollView` s PRÁZDNÝM obsahem se smrskne na nulovou šířku** a text v jeho `.overlay` se vysází po písmenech pod sebe (F18/M12, chytil snímek okna — čísla to vidět nemohla). `maxWidth: .infinity` patří na OBSAH scroll view, ne jen na něj samotný. Táž rodina pastí jako `aspectRatio(.fill)` níž: SwiftUI hlásí jinou velikost, než jakou člověk čeká, a pozná se to jen okem.
...
- **⚠️ ID objektů v `project.pbxproj` musí být UNIKÁTNÍ, a nikdo tě na duplicitu nevaruje srozumitelně.** Projekt používá čitelnou řadu `F181…`/`F182…` (soubor / build file), takže je svůdné si další prostě vymyslet. Když trefíš obsazené, Xcode obě položky **slije v jednu** a build spadne na hlášku, která ukazuje úplně jinam: `Build input file cannot be found: AIditor/AIditor/ThumbnailStore.swift` (soubor přitom existuje, jen o patro níž) plus varování „is a member of multiple groups… this indicates a malformed project". Stalo se 30. 07. 2026 při přidávání `Assets.xcassets`. **Před vložením si volné ID ověř:** `grep -o "F18[12]000000000000000000[0-9A-F]\{2\}" project.pbxproj | sort -u`, a po zásahu vždycky `plutil -lint project.pbxproj`.
- **Ikona se do appky dostává asset katalogem, ne volným `.icns`.** `AIditor/AIditor/Assets.xcassets/AppIcon.appiconset` (deset velikostí 16–512 včetně `@2x`) + `ASSETCATALOG_COMPILER_APPICON_NAME = AppIcon`. Že to opravdu vyšlo, se pozná z bundlu (`AppIcon.icns` a `Assets.car` v `Contents/Resources`, `CFBundleIconName = AppIcon`) — a nejlíp vytažením přes `NSWorkspace.icon(forFile:)`, což je přesně to, co ukáže Finder. **⚠️ `@2x` varianta nese detail své BODOVÉ velikosti, ne pixelové:** `icon_16x16@2x.png` má 32 px, ale musí vypadat jako šestnáctka, jinak vidí uživatel na retině jinou kresbu než na obyčejném displeji.
