...
- **`framesPerSegment = 8` jako výchozí, ale nastavitelné.** Spike 0 změřil, že **zvuk je na jemnosti segmentace nezávislý** — lupance nejsou ani při 545 segmentech. Když zvuk nerozhoduje, rozhoduje velikost kompozice: pětiminutový ramp při 30 fps je 9000 snímků, tedy **1 125 volání `scaleTimeRange` při osmi místo 9000 při jednom**. Skok rychlosti kolem 1 % je pod prahem viditelnosti.
  ⚠️ Skok závisí na délce klipu — na 40–70s klipech vyšel 0,96–1,12 %, ale na 11s klipu 3,79 %. **U krátkých záběrů může být potřeba jemnější hodnota, proto to nesmí být konstanta v kódu.**
- **Algoritmus korekce výšky patří do nastavení, ne natvrdo.** `.spectral` (výchozí, nejvyšší kvalita), `.timeDomain` (jiná charakteristika artefaktu, na některém materiálu lepší), `.varispeed` (bez korekce — výška se mění s rychlostí, občas žádaný efekt). Sedí na `AVAssetReaderTrackOutput.audioTimePitchAlgorithm`.
