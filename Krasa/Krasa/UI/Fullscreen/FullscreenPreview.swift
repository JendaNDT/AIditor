...
struct FullscreenPreviewOverlay: View {
    ...
    private var cleanCapsule: some View {
        HStack(spacing: 8) {
            ...
            Text("⎋ zpět do editoru")
                .font(.system(size: 10))
                .foregroundStyle(Color(krasaHex: 0xC9CCD1))
        }
        ...
        HStack(alignment: .top, spacing: 14) {
            ...
            HStack(spacing: 7) {
                if model.timeline.project.usesProxies {
                    chip("Proxy 1/2", color: Color(krasaHex: 0xC9CCD1))
                }
                ...
                if let loudness = model.lastLoudness {
                    chip(AppModel.decimal(loudness.lufs) + " LUFS",
                         color: KrasaUI.textSecondary, mono: true)
                }
                Text("⎋ zpět")
                    .font(.system(size: 10))
                    .foregroundStyle(KrasaUI.textPrimary)
    ...
    private func chip(_ text: String, color: Color, mono: Bool = false) -> some View {
        Text(text)
            .font(mono ? KrasaUI.Font.monoSmall : .system(size: 10))
    ...
    private var transport: some View {
        HStack(spacing: 12) {
            ...
            Button {
                ...
                ZStack {
                    Circle().fill(Color(krasaHex: 0xF2F3F5))
                    Image(systemName: model.controller.isPlaying ? "pause.fill" : "play.fill")
                        .font(.system(size: 18, weight: .bold))
                        .foregroundStyle(Color(krasaHex: 0x111315))
                }
                ...
                Text(model.controller.shuttleDescription)
                    ...
                    .background(RoundedRectangle(cornerRadius: 7)
                        .fill(model.controller.isSteppingFallback
                              ? KrasaUI.warn.opacity(0.16) : Color.white.opacity(0.08)))
            }
        }
    }
            ...
            ZStack(alignment: .topLeading) {
                ...
                if let range = rangeFraction {
                    RoundedRectangle(cornerRadius: 3)
                        .fill(KrasaUI.accent.opacity(0.28))
                        .frame(width: max(2, width * (range.upperBound - range.lowerBound)),
                               height: 10)
                        .overlay(HStack {
                            KrasaUI.accent.frame(width: 2)
                            Spacer(minLength: 0)
                            KrasaUI.accent.frame(width: 2)
                        })
                ...
                RoundedRectangle(cornerRadius: 2)
                    .fill(KrasaUI.playhead)
                    .frame(width: 4, height: 18)
                    .shadow(color: KrasaUI.playhead.opacity(0.2), radius: 0)
        ...
        Canvas { context, size in
            for fraction in marks {
                ...
                context.fill(Path(CGRect(x: x, y: 0, width: 1, height: size.height)),
                             with: .color(KrasaUI.warn.opacity(0.5)))
            }
        }
        .frame(height: 6)
