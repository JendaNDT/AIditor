...
//  Zdroj zadání: `design_handoff_krasa_ui/README.md`, obrazovka 6, stav `osa`.
//
    ...
    private var strip: some View {
        GeometryReader { geometry in
            ...
            .background(RoundedRectangle(cornerRadius: 10)
                .fill(Color(krasaHex: 0x0E0F11, opacity: 0.82)))
...
private struct MiniThumbStrip: View {
    ...
    var body: some View {
        ...
        .background(Color(krasaHex: 0x101216))
            ...
            for track in timeline.project.timeline.tracks where track.kind == .audio {
                for clip in track.clips {
                    ...
                    context.fill(Path(roundedRect: CGRect(x: x, y: 0, width: w,
                                                          height: size.height),
                                      cornerRadius: 2),
                                 with: .color(Color(krasaHex: 0x2A6950)))
                }
            }
        }
        .allowsHitTesting(false)
            ...
            for mark in timeline.project.beatMarks() {
                ...
                context.fill(Path(CGRect(x: x, y: mark.isDownbeat ? 0 : size.height * 0.35,
                                         ...
                                         height: mark.isDownbeat ? size.height
                                                                 : size.height * 0.65)),
                             with: .color(KrasaUI.warn.opacity(mark.isDownbeat ? 0.9 : 0.5)))
            }
        }
        .allowsHitTesting(false)
    }
        ...
        VStack(spacing: 5) {
            ZStack {
                RoundedRectangle(cornerRadius: 8)
                    .fill(LinearGradient(colors: [Color(krasaHex: 0x44525D),
                                                  Color(krasaHex: 0x181C22)],
