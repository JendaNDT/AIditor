...
struct LibraryCard: View {
    ...
    var body: some View {
        ...
        .background(isSelected ? KrasaUI.surfaceControlActive : KrasaUI.surfaceRow)
        .clipShape(RoundedRectangle(cornerRadius: 9))
        .overlay(
            RoundedRectangle(cornerRadius: 9)
                .strokeBorder(isSelected ? KrasaUI.accent : KrasaUI.border,
        ...
        ZStack {
            ...
            if let image = posterImage {
                ...
                Image(systemName: "waveform")
                    .font(.system(size: 20))
                    .foregroundStyle(KrasaUI.ok.opacity(0.7))
            }
        }
        ...
        VStack(spacing: 0) {
            HStack {
                ...
                badge(durationText, background: Color.black.opacity(0.6),
                      foreground: KrasaUI.textSecondary)
            }
            Spacer()
            HStack(alignment: .bottom, spacing: 3) {
                ...
                if asset.measuredFrameRate >= 100 {
                    badge("\(Int(asset.measuredFrameRate.rounded()))p",
                          background: Color(krasaHex: 0x78B4FF, opacity: 0.92),
                          foreground: Color(krasaHex: 0x10131A), bold: true)
                }
                if let timing, timing.isVariable {
                    badge("VFR", background: KrasaUI.warn.opacity(0.9),
                          foreground: Color(krasaHex: 0x1A1A1A), bold: true)
                }
                Spacer()
                if softRuns > 0 {
                    // Ryska, ne text: je to varování „tady se podívej", ne údaj.
                    RoundedRectangle(cornerRadius: 2)
                        .fill(KrasaUI.warn.opacity(0.9))
        ...
        VStack(alignment: .leading, spacing: 1) {
            ...
            Text(metaText)
                ...
                .foregroundStyle(asset.creationDateSource == .fileSystem
                                 ? KrasaUI.warn : KrasaUI.textTertiary)
                .lineLimit(1)
        }
