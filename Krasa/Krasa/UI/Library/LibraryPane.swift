...
//  Zdroj zadání: `design_handoff_krasa_ui/README.md`, sekce 1.3 „Knihovna".
//
        ...
        Button(action: action) {
            Text(count > 0 ? "\(title) \(count)" : title)
                ...
                .overlay(RoundedRectangle(cornerRadius: 12)
                    .strokeBorder(isOn ? KrasaUI.borderActive : KrasaUI.border, lineWidth: 1))
        }
        .buttonStyle(.plain)
    }
}
...
private struct ProxyFooterLine: View {
    ...
    var body: some View {
        HStack(spacing: 8) {
            Circle()
                .fill(proxies.progressText != nil ? KrasaUI.warn : KrasaUI.ok)
