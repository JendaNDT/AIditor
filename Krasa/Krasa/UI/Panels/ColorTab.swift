...
struct ColorTab: View {
    ...
    private func tile(image: NSImage?, label: String, highlighted: Bool) -> some View {
        VStack(alignment: .leading, spacing: 3) {
            ...
            .overlay(RoundedRectangle(cornerRadius: 6)
                .strokeBorder(highlighted ? KrasaUI.accent : KrasaUI.border, lineWidth: 1))
            Text(label)
        ...
        return Button {
            ...
            HStack(spacing: 8) {
                ...
                Text(preset?.displayName ?? "Bez úpravy")
                    ...
                    .foregroundStyle(isSelected ? KrasaUI.accent : KrasaUI.textPrimary)
                Spacer(minLength: 0)
                if isSelected {
                    Text("vybráno")
                        .font(KrasaUI.Font.chip)
            ...
            .background(RoundedRectangle(cornerRadius: 6)
                .fill(isSelected ? Color(krasaHex: 0x3A3320) : KrasaUI.surfaceRow))
            .overlay(RoundedRectangle(cornerRadius: 6)
                .strokeBorder(isSelected ? KrasaUI.accent : Color.clear, lineWidth: 1))
        }
        .buttonStyle(.plain)
    }
    ...
    private func swatch(_ preset: ColorPreset?) -> some View {
        ZStack {
            RoundedRectangle(cornerRadius: 3).fill(KrasaUI.surfaceWindow)
    ...
    private var selectionRow: some View {
        HStack(spacing: 8) {
            Text(selectedVideoCount > 1
                ...
                .font(KrasaUI.Font.chip)
                .foregroundStyle(selectedVideoCount > 1 ? KrasaUI.accent : KrasaUI.textTertiary)
                .padding(.horizontal, 7)
                .padding(.vertical, 3)
                .background(RoundedRectangle(cornerRadius: KrasaUI.Metric.chipRadius)
                    .fill(selectedVideoCount > 1
                          ? KrasaUI.accent.opacity(0.12) : Color.clear))
            Text("jeden krok ⌘Z")
                .font(KrasaUI.Font.chip)
                .foregroundStyle(KrasaUI.textTertiary)
            Spacer(minLength: 0)
