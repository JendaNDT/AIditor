    ...
    private func presetButton(title: String, speed: Double?) -> some View {
        ...
        return Button {
            ...
            Text(title)
                ...
                .foregroundStyle(isActive ? KrasaUI.accent
                                 : (belowLimit ? KrasaUI.textDisabled : KrasaUI.textPrimary))
                .frame(maxWidth: .infinity)
                .frame(height: KrasaUI.Metric.controlHeight)
                .background(RoundedRectangle(cornerRadius: KrasaUI.Metric.controlRadius)
                    .fill(isActive ? Color(krasaHex: 0x3A3320) : KrasaUI.surfaceControl))
                .overlay(RoundedRectangle(cornerRadius: KrasaUI.Metric.controlRadius)
                    .strokeBorder(isActive ? KrasaUI.accent : KrasaUI.borderStrong, lineWidth: 1))
        }
        .buttonStyle(.plain)
    ...
    private var editorBox: some View {
        VStack(alignment: .leading, spacing: 4) {
            RampEditorPaneView(controller: timeline)
                .frame(height: 150)
                .background(RoundedRectangle(cornerRadius: 8).fill(KrasaUI.surfaceWindow))
        ...
        HStack(spacing: 8) {
            if let clip, clip.speedRamp != nil,
               let plan = timeline.project.rampPlaybackPlan(of: clip) {
                chip("\(plan.segments.count) úseků", color: KrasaUI.accent)
                if plan.limitedByFrameRate {
                    chip("mez skoku nedosažitelná", color: KrasaUI.warn)
                }
            }
            if let clip {
    ...
    private func beatButton(title: String, failure: TimelineError?,
        ...
        Button(action: action) {
            Text(title)
                ...
                .foregroundStyle(failure == nil ? KrasaUI.textPrimary : KrasaUI.textDisabled)
