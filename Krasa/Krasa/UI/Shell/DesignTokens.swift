...
//  Zdroj: `design_handoff_krasa_ui/README.md`, sekce „Designové tokeny".
//
...
enum KrasaUI {
    ...
    static let surfaceWindow = Color(krasaHex: 0x0E0F11)
    static let surfaceChrome = Color(krasaHex: 0x17181B)
    static let surfaceRail = Color(krasaHex: 0x131417)
    static let surfacePanel = Color(krasaHex: 0x16181C)
    static let surfaceRow = Color(krasaHex: 0x1B1D21)
    static let surfaceControl = Color(krasaHex: 0x1E2024)
    static let surfaceControlActive = Color(krasaHex: 0x23262B)
    /// Aktivní ikona railu — o stupeň světlejší než vybraný přepínač.
    static let surfaceRailActive = Color(krasaHex: 0x2C3037)
    static let surfaceViewer = Color(krasaHex: 0x08090A)

    static let border = Color(krasaHex: 0x24262B)
    static let borderStrong = Color(krasaHex: 0x2F3238)
    static let borderActive = Color(krasaHex: 0x3A3F47)

    static let textPrimary = Color(krasaHex: 0xE9EAEC)
    static let textSecondary = Color(krasaHex: 0x9AA0A8)
    static let textTertiary = Color(krasaHex: 0x6B7280)
    static let textDisabled = Color(krasaHex: 0x5C6067)

    // MARK: - Barvy: signální (shodné s `TimelinePalette`, tmavá větev)

    /// Výběr, výřez, aktivní záložka. `clipSelectedStroke` (1,00 0,79 0,28).
    static let accent = Color(krasaHex: 0xFFC947)
    /// Text NA akcentu. Žlutá je světlá — bílý text by na ní zmizel.
    static let accentText = Color(krasaHex: 0x191A1C)
    static let playhead = Color(krasaHex: 0xFF453B)
    static let rampCurve = Color(krasaHex: 0x8CB8FF)
    /// Hotový stav, hlasitost v mezích.
    static let ok = Color(krasaHex: 0x3ECF8E)
    /// Běžící analýza, přiznaná mez. Totéž co `TimelinePalette.beat`.
    static let warn = Color(krasaHex: 0xFF9F0A)
...
extension Color {
    ...
    ///
    /// Pojmenovaný popisek `krasaHex`, ne prosté `hex:` — kdyby SwiftUI nebo
    /// jiný balíček někdy `Color(hex:)` doplnil, tichý překryv by se hledal
    /// dlouho.
    init(krasaHex value: UInt32, opacity: Double = 1) {
        self.init(.sRGB,
