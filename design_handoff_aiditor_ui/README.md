# Handoff: UI Projektu AIditor (macOS videoeditor)

## O čem to je

Přerovnání hlavního okna nativního macOS editoru **AIditor** (Swift, SwiftUI panely + AppKit timeline) do
...
**znovu postavit v existujícím prostředí aplikace AIditor**: SwiftUI pro panely, AppKit (`NSView` + `CALayer`)
...
`designs/AIditor hlavni okno.dc.html` · okno **1470×900** (MacBook Air 13"), minimum 1180×760.
...
V `designs/AIditor varianty 1a-3a.dc.html` jako varianta **3a** (kolo 3 nahoře).
...
`designs/AIditor export.dc.html` · **list 660 px** na střed (top 56) nad ztmaveným oknem
...
`designs/AIditor prepis.dc.html` · stejný rám jako hlavní okno, rail na **Řeč**, panel vpravo = přepis.
...
`designs/AIditor fullscreen aplikace.dc.html` · **1470×956** (celá obrazovka Airu 13").
...
`designs/AIditor fullscreen nahled.dc.html` · 1470×900, obraz 16:9 = pás **827 px** na střed (pillarbox 36 / 37).
...
| Soubor | Co obsahuje |
|---|---|
| `designs/AIditor hlavni okno.dc.html` | **hlavní směr** — hlavní okno 1470×900, panel s záložkami (`panelTab`), `showRampPanel` |
| `designs/AIditor fullscreen aplikace.dc.html` | totéž na celé obrazovce 1470×956 |
| `designs/AIditor fullscreen nahled.dc.html` | fullscreen náhled jen s obrazem, stavy `cisty/ovladani/osa` |
| `designs/AIditor export.dc.html` | list exportu, stavy `nastaveni/prubeh/hotovo` |
| `designs/AIditor prepis.dc.html` | přepis řeči, stavy `prubeh/editace` |
| `designs/AIditor varianty 1a-3a.dc.html` | historie rozhodování: **1a** rekonstrukce dnešního UI, **1b/1c** varianty layoutu, **2a** zvolená kombinace, **3a** prázdný start |
